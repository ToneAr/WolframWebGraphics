...
(* ::Section:: *) (* Interactive Constructs *)
serialize[HoldPattern[DynamicModule[_, body_, ___]], props_] :=
	serialize[body, props];
serialize[
	LightDarkSwitched[light_?ColorQ, PatternSequence[] | Automatic],
	props_
] :=
	serialize[Charting`DarkerLightDark[LightDarkSwitched[light], 0], props];
serialize[
	LightDarkSwitched[PatternSequence[] | Automatic, dark_?ColorQ],
	props_
] :=
	serialize[
		Charting`DarkerLightDark[LightDarkSwitched[Automatic, dark], 0],
		props
	];
serialize[LightDarkSwitched[light_?ColorQ, dark_?ColorQ], props_] :=
	Switch[$lightDark,
		"Light",
			serialize[light, props],
		"Dark",
			serialize[dark, props],
		_,
			ThrowException[
				ToWebGraphicsError,
				<|"Information" -> "Invalid $lightDark value"|>
			]
	];

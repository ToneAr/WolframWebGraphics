...
PackageScoped[
	{
		...
		colorSpecQ,
		normalizeColorSpec,
		$defaultTextAndAxesColor,
		colorToSvg,
		$emptyStyle,
		applyDirective,
		getCurrentPointRadius,
		getCurrentStyleProps,
		getCurrentTextProps,
		getCurrentFilling,
		getCurrentArrowheads,
		optionsToGlobalSvgProps
	}
];
...
$defaultTextAndAxesColor = GrayLevel[0.4];
...
colorToSvg[col_?colorSpecQ] :=
	Module[{
		...
		<|
			"rgb"   -> StringJoin[
				"#",
				IntegerString[Clip[#, {0, 255}], 16, 2]& /@ {r, g, b}
			],
			"alpha" -> a
		|>
	];
...
symbolicImage[Tiny] = 100.;
symbolicImage[Small] = 180.;
symbolicImage[Medium] = 360.;
symbolicImage[Large] = 720.;
symbolicImage[_] = 360.;
...
emitFill[a_, col_, op_] :=
	If[
		MissingQ[col],
		(* No explicit fill colour: SVG (like WL) defaults the fill to black, so
		   no "fill" attribute is needed -- but a bare Opacity directive must
		   still apply to that default fill (e.g. a GeoGraphics region highlight
		   `Style[Polygon[...], Opacity[0.3]]`), otherwise it renders solid. *)
		With[{o = op /. _Missing -> 1.},
			If[o < 1, <|a, "fill-opacity" -> makeSvgNumber[o]|>, a]
		],
	...
	Module[{
		...
		a =
			emitFill[
				...
				chooseMissing[
					st["FontColor"],
					chooseMissing[st["Color"], $defaultTextAndAxesColor]
				],
				st["Opacity"]
			];
		a["font-size"] =
...
axisLabelRightExtra[o_, w_, base_] :=
	Module[{
			labels = Lookup[o, AxesLabel, None],
			width
		},
...
tickMarginSpecPairQ[x_] := ListQ[x] && Length[x] === 2 && !tickMarginTupleQ[x];
...
axisMarginTickSpec[spec_, i_] := If[tickMarginSpecPairQ[spec], spec[[i]], spec];
...
tickMarginOne[tick_, side_, styles_] :=
	Module[{
		...
		normal = Max[outside, labelOffset + tickSideNormalSize[side, size]];
		tangent =
			If[tickLabelMissingQ[tick[[2]]], 0, tickSideTangentSize[side, size] / 2];
		Switch[side,
			"Bottom",
...
axisOriginForMargins[o_, {{x1_, x2_}, {y1_, y2_}}] :=
	Module[{
			origin = Lookup[o, AxesOrigin, Automatic]
		},
...
constrainMargins[{l_, r_, b_, t_}, w_, h_] :=
	Join[constrainMarginPair[{l, r}, w], constrainMarginPair[{b, t}, h]];

paddingPair[{a_?NumericQ, b_?NumericQ}] := {a, b};
paddingPair[n_?NumericQ] := {n, n};
paddingPair[_] := {0., 0.};

plotRangePaddingPairs[{{xl_, xr_}, {yb_, yt_}}] :=
	{paddingPair[{xl, xr}], paddingPair[{yb, yt}]};
plotRangePaddingPairs[{x_, y_}] := {paddingPair[x], paddingPair[y]};
plotRangePaddingPairs[n_?NumericQ] := {{n, n}, {n, n}};
plotRangePaddingPairs[_] := {{0., 0.}, {0., 0.}};

expandRangeByPadding[{a_, b_}, {lo_, hi_}] := {a - lo, b + hi};

imagePaddingMargins[{{l_?NumericQ, r_?NumericQ}, {b_?NumericQ, t_?NumericQ}}] :=
	{l, r, b, t};
imagePaddingMargins[_] := {0., 0., 0., 0.};
...
optionsToGlobalSvgProps[opts_List] :=
	Module[{
		...
		If[MatchQ[pr, {{_?NumericQ, _?NumericQ}, {_?NumericQ, _?NumericQ}}],
			prPadding = plotRangePaddingPairs[Lookup[o, PlotRangePadding, None]];
			xr = expandRangeByPadding[pr[[1]], prPadding[[1]]];
			yr = expandRangeByPadding[pr[[2]], prPadding[[2]]]
		];
		...
		baseMargins = If[axisOn, 1, 0] * {0.12 * w, 0.04 * w, 0.1 * h, 0.05 * h};
		labelMargins =
			{
		...
		tickMargins =
			If[ListQ[xr], tickDecorationMargins[o, {xr, yr}], {0, 0, 0, 0}];
		imageMargins = imagePaddingMargins[Lookup[o, ImagePadding, None]];
		margins =
			MapThread[
				Max,
				{
					MapThread[Max, {baseMargins, tickMargins}] + labelMargins,
					imageMargins
				}
			];
		margins = constrainMargins[margins, w, h];
		(*
		 * Fit the data range into the pixel box.
		...
		(*
		 * Pass through custom options as data-<name>
		 * The standard layout / styling options below are consumed or are noise
		 *)
		rest =
			KeyDrop[
				o,
